...
Imager.prototype.resizeFile = function (file, preset, fname, cb) {
  ...
  gm(file['path'])
    ...
    .buffer(function(err, buffer) {
      ...
      self['pushTo' + self.storage](buffer, remoteFile, filename, file['type'], cb)
    })
}
...
Imager.prototype.pushToRackspace = function (buffer, remoteFile, filename, type, cb) {
  ...
  var addFile = function (err, container) {
    if (err) return cb(err)
    ...
    client.addFile(rsContainer, options, onUpload)

    // emit the data and end events
    img.emit('data', buffer)
    img.emit('end')
  }

  client.setAuth(function() {
    client.getContainer(rsContainer, true, addFile)
  })
}


Imager.prototype.pushToS3 = function (buffer, remoteFile, filename, type, cb) {
  var self = this
  ...
  client.putStream(img, remoteFile, headers, function (err, res) {
    if (err) return cb(err)
    if (self.config.debug) console.log(remoteFile + ' uploaded')
