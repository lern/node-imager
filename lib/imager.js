...
Imager.prototype.upload = function (files, callback, variant, keepName) {
  var self = this
  ...
  var startUpload = function (file, iteratorCallback) {
    var filename
    if (keepName) {
      filename = file.name.replace(/(\.jpg|\.png|\.gif)/g,'')
    } else {
      filename = Math.round(new Date().getTime())
    }
    self.startUpload(file, filename, variant, iteratorCallback)
  }

  var flowCtrl = async.forEach
  if (self.config.parallelImageProcessing == false) {
    flowCtrl = async.forEachSeries
    if (self.config.debug) console.log('Processing images in series')
  }

  flowCtrl(files, startUpload, function (err) {
...
Imager.prototype.startUpload = function (file, filename, variant, iteratorCallback) {
  ...
  if (variants.fit) {
    Object.keys(variants.fit).forEach(function (name) {
      var processFiles = function (cb) {
        self.fitFile(file, { name: name, size: variants.fit[name] }, filename, cb)
      }
      asyncArr.push(processFiles)
    })
  }

  var flowCtrl = async.parallel
  if (self.config.parallelVariantProcessing == false) {
    flowCtrl = async.series
    if (self.config.debug) console.log('Processing variants in series')
  }

  flowCtrl(asyncArr, function (err, results) {
...
Imager.prototype.resizeFile = function (file, preset, fname, cb) {
  ...
  if (this.config.autoOrient == false) {
    gm(file['path'])
...
Imager.prototype.fitFile = function (file, preset, fname, cb) {
  var ct = { 'image/jpeg': '.jpg', 'image/png': '.png', 'image/gif': '.gif' }
    , fileType = ct[file['type']]
    , filename = fname + fileType
    , self = this
  ...
  if (this.config.autoOrient == false) {
    gm(file['path'])
      .thumbnail(preset.size.split('x')[0], '' + preset.size.split('x')[1] + '^')
      .gravity('Center')
      .extent(preset.size.split('x')[0], preset.size.split('x')[1])
      .noProfile()
      .write(dest, function(err) {
        if (err) return cb(err)
        self['pushTo' + self.storage](dest, remoteFile, filename, file['type'], cb)
...
Imager.prototype.prepareRemove = function (file, iteratorCallback, variant) {
  ...
  if (variants.fit) {
    Object.keys(variants.fit).forEach(function (preset) {
      var removeFiles = function (cb) {
        self['removeFrom' + self.storage](file, preset, cb)
      }
